...
import { FINOS_CATEGORIES, fetchMarketplaceCategories } from '../../utils/categories';
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setCategories(list);
        if (list.length && !list.find((c) => c.id === category)) {
    ...
    <div className="demo-engine">
      ...
      <div className="filter-row" style={{ display: 'flex', gap: '12px', alignItems: 'flex-end', flexWrap: 'wrap' }}>
        ...
        <div className="form-group" style={{ flex: '0 0 auto' }}>
          <button className="btn btn-primary" onClick={() => setCategory((c) => c)}><i className="fas fa-search"></i> Compare</button>
        </div>
      </div>
